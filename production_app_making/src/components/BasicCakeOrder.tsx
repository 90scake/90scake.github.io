...
import { BasicCake, Flavor, Design, OrderDetails } from '../types';
import { sendWhatsAppMessage } from '../utils/whatsapp';

interface BasicCakeOrderProps {
  onClose: () => void;
}

const BasicCakeOrder: React.FC<BasicCakeOrderProps> = ({ onClose }) => {
  const [basicCake, setBasicCake] = useState<BasicCake | null>(null);
  ...
  const calculateTotalPrice = () => {
    ...
    const selectedFlavorObj = basicCake.flavors.find(f => f.id === selectedFlavor);
  ...
  const getDesignName = (id: string): string => {
    if (!basicCake) return '';
    return basicCake.designs.find(d => d.id === id)?.name || '';
  };
  
  const getDesignImage = (id: string): string => {
    if (!basicCake) return '';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center overflow-y-auto p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-800">Order {basicCake.name}</h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <h3 className="font-semibold text-lg mb-4">1. Select Your Flavor</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {basicCake.flavors.map((flavor: Flavor & { priceAdjustment?: number }) => (
                  <label 
                    key={flavor.id}
                  ...
                  >
                    ...
                    {flavor.priceAdjustment ? (
                      <span className="text-sm text-gray-600">+{flavor.priceAdjustment} BDT</span>
                    ) : null}
                  </label>
                ))}
              </div>
            ...
            <div>
              <h3 className="font-semibold text-lg mb-4">2. Choose Your Design</h3>
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-pink-600">{totalPrice} BDT</span>
              </div>
            </div>
            
            <div>
              <h3 className="font-semibold text-lg mb-4">3. Your Information</h3>
