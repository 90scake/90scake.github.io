...
const CakeModal = ({ cake, onClose }) => {
  ...
  const [selectedFlavor, setSelectedFlavor] = useState(cake.flavors[0]?.id || '');
  const [selectedPound, setSelectedPound] = useState(1);
  const modalRef = useRef(null);
  
  // Calculate current price based on selections
  const calculatePrice = () => {
    const priceObj = cake.prices.find(p => p.flavorId === selectedFlavor && p.pound === selectedPound);
            ...
            <div className="space-y-6">
                    ...
                    <tbody>
                      {cake.prices.map((price, index) => (
                        <tr key={index} className="border-b border-gray-100 last:border-0">
                          <td className="py-2">{getFlavorName(price.flavorId)}</td>
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  <div>
                    <label htmlFor="pound" className="block text-sm font-medium text-gray-700 mb-1">
                      Choose Size
                    </label>
                    <select
                      id="pound"
                      ...
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-pink-500 focus:border-pink-500"
                    >
                      {Array.from(new Set(cake.prices.map(p => p.pound))).map(pound => (
                        <option key={pound} value={pound}>{pound} pound</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">Selected: {getFlavorName(selectedFlavor)}, {selectedPound} pound</p>
                    <p className="text-xl font-bold text-pink-600 mt-1">Total: {calculatePrice()} BDT</p>
                  </div>
